...
from h_test_IQM.metrics.SSIM.ssim import ms_ssim
...
from h_test_IQM.metrics.NLPD_torch.pyramids import LaplacianPyramid

from brisque import BRISQUE as brisque_pkg
...
class BRISQUE:
    ''' using code from https://github.com/rehanguha/brisque'''
    def __init__(self):
        self.obj = brisque_pkg(url=False)
    
    def __call__(self, im_comp, **kwargs):
        return self.obj.score(img=im_comp)
...
class BRISQUE_diff:
    ...
    def __call__(self, im_ref, im_comp, **kwargs):
        _check_shapes(im_ref, im_comp)
        # I think we want this as a negative value is informative for BRISQUE??? (i.e. distorted image is higher quality than the original)
        return self.obj.score(img=im_comp) - self.obj.score(img=im_ref)
